...
class LinearizedNetwork:
    ...
    def __init__(self, bus_ids: List[str], base_voltages: np.ndarray,
                 slack_bus: str = None):
    ...
    def estimate_voltages(self, base_voltages: np.ndarray,
                          ev_loads_per_bus: Dict[str, float]) -> np.ndarray:
    ...
    def check_voltage_limits(self, voltages: np.ndarray,
                              v_min: float = 0.95, v_max: float = 1.05) -> bool:
        """Return True if all voltages are within [v_min, v_max]."""
        return np.all((voltages >= v_min) & (voltages <= v_max))

    def get_voltage_violations(self, voltages: np.ndarray,
                                v_min: float = 0.95, v_max: float = 1.05) -> Dict[str, float]:
...
def create_simple_network(evcs_buses: List[str], num_buses: int = 10) -> LinearizedNetwork:
