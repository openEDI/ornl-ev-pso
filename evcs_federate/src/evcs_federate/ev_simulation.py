"""PSO optimization and battery physics engine for EV charging scheduling."""
...
def fitness_function(charging_rate, network, base_voltages, evcs_bus, ev_params):
    ...
    for t in range(num_control_steps):
        ...
        for bus, ev_indices in evcs_bus_assignment.items():
            ev_loads_per_bus[bus] = np.sum(real_rate[ev_indices, t]) if ev_indices else 0.0

        if network is not None:
            estimated_voltages = network.estimate_voltages(base_voltages, ev_loads_per_bus)
            violations = np.maximum(0, v_min - estimated_voltages) + \
                         np.maximum(0, estimated_voltages - v_max)
...
def ev_pso_optimization(num_particles, max_iterations, network, base_voltages,
                         evcs_bus, ev_params):
    ...
    logger.info(f"EVs: {num_evs}, Time steps: {num_control_steps}, Max rate: {max_charging_rate} kW")
    logger.info(f"Target SOC: {desired_state_of_charge*100:.0f}%")
    logger.info(f"Initial SOC range: {np.min(initial_soc)*100:.1f}% - {np.max(initial_soc)*100:.1f}%")
    ...
    logger.info(f"Initial best fitness: {global_best_fitness:.2f} (Particle {global_best_index})")
    ...
    for iteration in range(max_iterations):
        for i in range(num_particles):
            ...
            current_fitness = fitness_function(particles[i], network, base_voltages,
                                                   evcs_bus, ev_params)
        ...
        if (iteration + 1) % 5 == 0 or iteration == max_iterations - 1:
            logger.info(f"Iter {iteration + 1:2d}/{max_iterations}: Best={global_best_fitness:.2f}, Inertia={inertia_weight:.3f}")
