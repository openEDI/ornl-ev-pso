...
def generate_ev_parameters(config=None):
    ...
    for station_idx, n_evs in enumerate(num_evs_per_station):
        ...
        arrival_hr = truncnorm.rvs(
            a_arr, b_arr, loc=arr_mean, scale=arr_std, size=n_evs
        )
        ...
        initial_soc_station = truncnorm.rvs(
            a_soc, b_soc, loc=soc_mean, scale=soc_std, size=n_evs
        )
