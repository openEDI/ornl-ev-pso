...
def xarray_to_powers(data, **kwargs):
    """Turn xarray into PowersReal and PowersImaginary."""
...
def get_powers(PQ_load, PQ_PV, PQ_gen, PQ_cap):
    """Turn xarrays into PowersReal and PowersImaginary."""
...
def get_initial_data(sim: FeederSimulator, config: FeederConfig):
    ...
    # Snapshot run required to re-enable elements properly before solving.
    sim.snapshot_run()
...
def where_power_unbalanced(PQ_injections_all, calculated_power, tol=1):
    """Find buses where PQ_injections does not match calculated power."""
...
def go_cosim(
    ...
    ev_load_real_key = (
        "unused/ev_load_real" if "ev_load_real" not in input_mapping else input_mapping["ev_load_real"]
    )
    sub_ev_load_real = vfed.register_subscription(ev_load_real_key, "")
    sub_ev_load_real.set_default("[]")
    sub_ev_load_real.option["CONNECTION_OPTIONAL"] = True

    ev_load_imag_key = (
        "unused/ev_load_imag" if "ev_load_imag" not in input_mapping else input_mapping["ev_load_imag"]
    )
    sub_ev_load_imag = vfed.register_subscription(ev_load_imag_key, "")
    sub_ev_load_imag.set_default("[]")
    sub_ev_load_imag.option["CONNECTION_OPTIONAL"] = True
    ...
    while request_time < int(config.number_of_timesteps):
        ...
        current_index = int(granted_time)
        ...
        # Support both pydantic v1 (__root__) and v2 (root)
        try:
        ...
        # Apply EV load if subscription has valid data (not empty default)
        ev_load_applied = False
        ...
        # Remove EV injection after solving to prevent accumulation across timesteps
        if ev_load_applied:
            sim.remove_power_injection(ev_load_real, ev_load_imag)
            logger.debug(f"Removed EV injection after timestep")
...
def run_simulator(broker_config: BrokerConfig):
    """Load static_inputs and input_mapping and run simulation."""
